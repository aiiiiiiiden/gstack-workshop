...
이 시점에서 gstack이 설치되어 있는지를 먼저 확인합니다.
...
이 시리즈는 gstack 0.16.4.0을 전제합니다. 버전이 다르면 이후 편에서 스킬 이름이나 프리앰블 동작이 다를 수 있습니다.
